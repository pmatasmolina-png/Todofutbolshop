...
document.getElementById("passwordChangeForm").addEventListener("submit",async e=>{e.preventDefault();const msg=document.getElementById("passwordMsg");const current=document.getElementById("currentAdminPassword").value;const next=document.getElementById("newAdminPassword").value;const next2=document.getElementById("newAdminPassword2").value;if(next!==next2){msg.textContent="Las nuevas contraseñas no coinciden.";return}try{await api("/api/admin-password",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({currentPassword:current,newPassword:next})});msg.textContent="Contraseña cambiada correctamente.";e.target.reset()}catch(err){msg.textContent=err.message}});
...
async function load(){await loadGalleryAdmin();catalogs=await api("/api/catalogs");renderCatalogEditor();discountCodes=await api("/api/discount-codes");renderDiscountCodes();let orders;try{orders=await api("/api/orders")}catch{return}if(!orders.length){document.getElementById("orders").innerHTML='<div class="empty">Todavía no hay pedidos.</div>';return}document.getElementById("orders").innerHTML=orders.map(o=>`<article class="order"><div class="order-top"><div><strong>Pedido #${o.id}</strong><br>${new Date(o.createdAt).toLocaleString("es-ES")}</div><div><span class="status">${escapeHtml(o.status)}</span><select onchange="changeStatus('${o.id}',this.value)">${["Pendiente","Confirmado","En preparación","Enviado","Entregado","Cancelado"].map(s=>`<option ${s===o.status?"selected":""}>${s}</option>`).join("")}</select></div></div><p><strong>${escapeHtml(o.customer.name)}</strong> · ${escapeHtml(o.customer.phone)}<br>${escapeHtml(o.customer.address||"Sin dirección")}</p><p><strong>${escapeHtml(o.catalogName||"Catálogo")}</strong> · ${o.quantity} camiseta(s) × ${Number(o.price).toFixed(2)} € = <strong>${Number(o.total).toFixed(2)} €</strong>${o.discount?` · Descuento total: -${Number(o.discount).toFixed(2)} €`:""}${o.discountCode?` · Código: ${escapeHtml(o.discountCode)}`:""}</p>${o.notes?`<p>📝 ${escapeHtml(o.notes)}</p>`:""}<div class="thumbs">${o.images.map(i=>`<div class="admin-thumb"><a href="${i.url}" target="_blank"><img src="${i.url}" alt="Camiseta"></a><strong>Talla: ${escapeHtml(i.size||"No indicada")}</strong><span>Cantidad: ${Number(i.quantity||1)}</span></div>`).join("")}</div></article>`).join("")}
